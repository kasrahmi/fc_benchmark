...
func (f *FirecrackerVM) StopVM() {
    ...
    cmd := exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@172.16.0.2", "reboot")
    if err := cmd.Run(); err != nil {
...
func (f *FirecrackerVM) RunCommandOnVM(command string) {
    ...
    cmd := exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@172.16.0.2", command)
    if err := cmd.Run(); err != nil {
...
func (f *FirecrackerVM) RunCommandOnVMTmux(command string) {
    ...
    exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@172.16.0.2", "tmux kill-session -t server").Run()
    time.Sleep(1 * time.Second)
    exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@172.16.0.2", "tmux new -s server -d").Run()
    time.Sleep(1 * time.Second)
    exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@172.16.0.2", "tmux send -t server", fmt.Sprintf(`"%s"`, command), "ENTER").Run()
}
