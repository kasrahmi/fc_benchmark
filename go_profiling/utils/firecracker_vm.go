...
func (f *FirecrackerVM) StopFCVMM() {
	...
	if err := cmd.Run(); err != nil {
		log.Warnf("Failed to stop VMM: %v", err)
	}
}
...
func (f *FirecrackerVM) StopVM() {
	...
	cmd := exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@"+FCVmIp, "reboot")
	if err := cmd.Run(); err != nil {
		log.Warnf("Failed to stop VM: %v", err)
	}
}
...
func (f *FirecrackerVM) RunCommandOnVM(command string) {
	...
	cmd := exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@"+FCVmIp, command)
	if err := cmd.Run(); err != nil {
...
func (f *FirecrackerVM) RunCommandOnVMTmux(command string) {
	...
	exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@"+FCVmIp, "tmux kill-session -t server").Run()
	time.Sleep(1 * time.Second)
	exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@"+FCVmIp, "tmux new -s server -d").Run()
	time.Sleep(1 * time.Second)
	exec.Command("ssh", "-i", UbuntuPath+"/ubuntu-22.04.id_rsa", "root@"+FCVmIp, "tmux send -t server", fmt.Sprintf(`"%s"`, command), "ENTER").Run()
}
